...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://karanaldo-07.github.io", "http://localhost:3000", "http://127.0.0.1:5500"],
...
def gemini_json(prompt: str, schema: dict):
    ...
    body = {"contents":[{"parts":[{"text":prompt}]}],"generationConfig":{"temperature":0.2,"responseMimeType":"application/json","responseSchema":schema}}
    try:
        response = requests.post(url, headers={"x-goog-api-key":GEMINI_API_KEY,"Content-Type":"application/json"}, json=body, timeout=25)
...
Be fair to junior candidates. Concise answers can score well when correct. Do not invent facts. For coding/technical questions prioritize correctness and reasoning; for behavioral questions prioritize the candidate's own actions and outcomes."""
    ...
    for key, cap in [("total",100),("relevance",30),("completeness",30),("structure",20),("specificity",20)]: result[key]=max(0,min(cap,int(result[key])))
    result["source"]="gemini"; result["model"]=GEMINI_MODEL
    return result
...
Rules: adapt difficulty to the answer; strong answers get a deeper follow-up or harder adjacent topic; weak answers get a focused clarifying question or simpler foundation check; never repeat; stay role/JD relevant; mix technical, coding/problem-solving, system-design and behavioral topics; target clear weaknesses; keep answerable in 1-3 minutes; return only JSON."""
    result=gemini_json(prompt,NEXT_SCHEMA); result["source"]="gemini"; result["model"]=GEMINI_MODEL
    return result
...
def report(req: ReportRequest, request: Request):
    ...
    prompt=f"""You are the senior interviewer producing a final candidate report after a mock interview. Return ONLY JSON matching the schema.
Role: {req.role}
Job description: {req.job_description[:7000]}
...
Rules: base conclusions on supplied evaluations; do not invent candidate facts; readiness should reflect overall performance; identify recurring weak dimensions and turn them into concrete role/JD-relevant revision topics; make the action plan practical and ordered; next questions should target weak areas; be concise and honest."""
    result=gemini_json(prompt,REPORT_SCHEMA); result["source"]="gemini"; result["model"]=GEMINI_MODEL
    return result
