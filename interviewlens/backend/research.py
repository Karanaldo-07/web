...
def search_web(role: str, company: str = ''):
    ...
    if not key:
        return {'enabled':False,'message':'Web research is ready but BRAVE_SEARCH_API_KEY is not configured.','sources':[],'question_leads':[]}
    ...
    for q in queries:
        try:
            r=requests.get(BRAVE_URL,params={'q':q,'count':10,'country':'IN','search_lang':'en','safesearch':'moderate','extra_snippets':'true'},headers={'Accept':'application/json','X-Subscription-Token':key},timeout=10)
            r.raise_for_status()
            for item in r.json().get('web',{}).get('results',[]):
                url=item.get('url')
                if url and not any(s['url']==url for s in sources):
                    sources.append({'title':item.get('title',''),'url':url,'snippet':item.get('description',''),'query':q,'extra_snippets':item.get('extra_snippets',[])})
